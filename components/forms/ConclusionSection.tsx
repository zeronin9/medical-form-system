    ...
    <div className={cardClass}>
      ...
      <div className={cardContentClass}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
          ...
          {(isActive('restrictions') || isActive('free_cond') || isActive('action_taken')) && (
            <div className="space-y-4">
              ...
              {isActive('restrictions') && formData.restrictions === 'Yes' && isActive('rest_desc') && (
                <div>
        ...
        <div className="border-t border-slate-100 pt-6">
          ...
          {(isActive('id_checked') || isActive('watch_able')) && (isIlo || isMlc) && (
            <div className="bg-blue-50/50 p-4 rounded-lg border border-blue-100 mb-6 space-y-4">
              <h4 className="font-semibold text-sm text-blue-900 border-b border-blue-200 pb-2">Administrasi Khusus ILO <BadgeILO/></h4>

              {isActive('id_checked') && (
                <div className="flex flex-col md:flex-row justify-between gap-2">
                  <span className="text-sm text-slate-700">Dokumen identitas (KTP/Paspor) diperiksa saat ujian?</span>
                  <div className="flex gap-4 shrink-0">
                    <label className={radioGroupClass}><input type="radio" name="id_checked" value="Yes" checked={formData.id_checked === 'Yes'} onChange={handleChange} className={radioClass} /><span>Ya</span></label>
                    <label className={radioGroupClass}><input type="radio" name="id_checked" value="No" checked={formData.id_checked === 'No'} onChange={handleChange} className={radioClass} /><span>Tidak</span></label>
                  </div>
                </div>
              )}
              ...
              {isActive('watch_able') && (
                <div className="flex flex-col md:flex-row justify-between gap-2">
                  <span className="text-sm text-slate-700">Mampu untuk tugas jaga (Able for watchkeeping)?</span>
                  <div className="flex gap-4 shrink-0">
                    <label className={radioGroupClass}><input type="radio" name="watch_able" value="Yes" checked={formData.watch_able === 'Yes'} onChange={handleChange} className={radioClass} /><span>Ya</span></label>
                    <label className={radioGroupClass}><input type="radio" name="watch_able" value="No" checked={formData.watch_able === 'No'} onChange={handleChange} className={radioClass} /><span>Tidak</span></label>
                  </div>
                </div>
              )}
            </div>
