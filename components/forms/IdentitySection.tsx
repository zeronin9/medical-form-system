    ...
    <div className={cardClass}>
      ...
      <div className={cardContentClass}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {isActive('firstName') && (
            <div>
              <label className={labelClass}>Nama Depan <BadgeADNOC /><BadgeMarshall /></label>
          ...
          {isActive('middleName') && (
            <div>
              <label className={labelClass}>Nama Tengah <BadgeADNOC /><BadgeMarshall /></label>
              <input type="text" name="middleName" value={formData.middleName || ''} onChange={handleChange} className={inputClass} placeholder="Middle Name (Opsional)" />
            </div>
          )}

          {isActive('familyName') && (
            <div>
              <label className={labelClass}>Nama Belakang <BadgeADNOC /><BadgeMarshall /></label>
              <input type="text" name="familyName" value={formData.familyName || ''} onChange={handleChange} className={inputClass} placeholder="Family/Last Name" />
            </div>
          )}

          {isActive('idPassport') && (
            <div>
              <label className={labelClass}>No. KTP / Paspor</label>
              <input type="text" name="idPassport" value={formData.idPassport || ''} onChange={handleChange} className={inputClass} />
            </div>
          )}
          ...
          {isActive('nationality') && (
            <div>
              <label className={labelClass}>Kewarganegaraan {isMlc && <BadgeMLC />}</label>
          ...
          {isActive('company') && (
            <div>
              <label className={labelClass}>Nama Perusahaan {isMlc && <BadgeMLC />}</label>
          ...
          {isActive('position') && (
            <div>
              <label className={labelClass}>Posisi / Jabatan (Selain Pelaut)</label>
          ...
          {isActive('workLocation') && (
            <div>
              ...
              <input type="text" name="workLocation" value={formData.workLocation || ''} onChange={handleChange} className={inputClass} />
            </div>
          )}

          {isActive('contactNumber') && (
            <div>
              <label className={labelClass}>No. Telepon / HP</label>
          ...
          {isActive('email') && (
            <div>
              <label className={labelClass}>Email Address <BadgeADNOC /></label>
          ...
          {isActive('serviceDate') && (
            <div>
              <label className={labelClass}>Tanggal Mulai Kerja <BadgeChevron /></label>
              <input type="date" name="serviceDate" value={formData.serviceDate || ''} onChange={handleChange} className={inputClass} />
            </div>
          )}

          {isActive('medNo') && (
            <div>
              <label className={labelClass}>No. Rekam Medis <BadgeChevron /></label>
              <input type="text" name="medNo" value={formData.medNo || ''} onChange={handleChange} className={inputClass} />
            </div>
          )}

          {isActive('department') && (
            <div>
              <label className={labelClass}>Departemen {isQatar && <BadgeQatar />} {isMlc && <BadgeMLC />}</label>
          ...
          {showMaritimeSection && (
            <>
              ...
              {isActive('pob_city') && (
                <div>
                  <label className={labelClass}>Kota Lahir <BadgeILO /><BadgeMLC /><BadgeMarshall /></label>
                  <input type="text" name="pob_city" value={formData.pob_city || ''} onChange={handleChange} className={inputClass} placeholder="Contoh: Surabaya" />
                </div>
              )}

              {isActive('pob_country') && (
                <div>
                  <label className={labelClass}>Negara Lahir <BadgeILO /><BadgeMLC /><BadgeMarshall /></label>
                  <input type="text" name="pob_country" value={formData.pob_country || ''} onChange={handleChange} className={inputClass} placeholder="Contoh: Indonesia" />
                </div>
              )}

              {isActive('seaman_book') && (
                <div>
                  <label className={labelClass}>Buku Pelaut (Seaman Book) {isIlo && <BadgeILO />} {isMlc && <BadgeMLC />}</label>
                  <input type="text" name="seaman_book" value={formData.seaman_book || ''} onChange={handleChange} className={inputClass} />
                </div>
              )}
          ...
          {isActive('job1') && isAdnoc && (
            <div className="col-span-full mt-6 rounded-lg border border-slate-200 bg-slate-50/30 p-4">
              <h4 className="text-sm font-bold text-slate-900 mb-4">
                Riwayat Pekerjaan Sebelumnya (Previous Employment) <BadgeADNOC />
              </h4>
